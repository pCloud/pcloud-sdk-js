...
one((method, { params: { folderid }}) => method === 'createfolder' && folderid === 1, success(mockReturnCreatefolder), createdMock);
one((method, { params: { folderid }}) => method === 'createfolder' && folderid === 1337, error(2004, "File or folder already exists."), alreadyExistsMock);
...
describe('throws error false when unsuccessful', async () => {
  it('handles the error correctly', async () => {
    const response = await createfolder('folder name', 1337)
      .catch((error) => error);
    ...
    expect(response.result).toBe(2004);
		...
		expect(apiMethod.mock.calls[1][1].params).toEqual({ access_token: 'testauth', name: 'folder name', folderid: 1337 });
  });
});
