...
on(
	(method, options) => options.apiServer === "broken-api.pcloud.com",
	error(500, "Network Error."),
	receivedNetworkError
);
on(
	(method, options) => method === "listfolder" && options.apiServer === "api.pcloud.com",
	success({ metadata: { name: "/", folderid: 0 } }),
	retried
);
